...
for year in range(2016,2024):
    with open('./data/vehicle_data/' +str(year) +'.json', 'a') as outfile:
        for mon in range(1,13):
            for dateT in list(c.itermonthdates(year, mon))[:monthrange(year, mon)[1]]:
                if dateT < date(2021, 12, 31) :  
                    ...
                    response = requests.get('https://data.cityofnewyork.us/resource/bm4k-52h4.json',params=params, auth=basic)
